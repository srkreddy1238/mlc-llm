...
build_model() {
    ...
    python3 -m mlc_llm compile ${MODEL_ARTIFACTS_PATH}/dist/${model}-${quantization}-MLC/mlc-chat-config.json --device android:cl-adreno-so --opt "openclml=1" -o ${MODEL_ARTIFACTS_PATH}/dist/libs/${model}-${quantization}-adreno-opencl-clml.so
    if [ $ACCL -eq 1 ] ; then
