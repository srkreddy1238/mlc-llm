...
from apps.maths500.evaluateMaths500 import evalMaths500
...
class ChatState:
    ...
    def chat(self, maths500=False, eval_data_path=None):
        ...
        while True:
            if not maths500:
                prompt = get_prompt(
                    ">>> ", key_bindings=kb, multiline=True  # pylint: disable=protected-access
                )
            else:
                with open(eval_data_path, "r", encoding="utf-8") as f:
                    all_prompts = f.read().strip()
                # Split prompts using the separator ---
                prompts = all_prompts.split("\n\n---\n\n")  # Ensures each block is kept intact
                with open("maths500Output.txt", "w", encoding="utf-8") as fout:
                    for prompt in prompts:
                        self.reset()
                        self.generate(prompt)
                        response = self.history[-1]["content"]
                        fout.write("=== RESPONSE START ===\n")
                        fout.write(response.strip() + "\n")
                        fout.write("=== RESPONSE END ===\n\n")
...
def chat(
    ...
    if maths500:
        ...
        chat_state.overrides.max_tokens = 4096
        chat_state.chat(maths500, eval_data_path)
    else:
        chat_state = ChatState(engine)
        chat_state.chat()
