...
from .qwen35_moe_model import Qwen35MoeConfig, Qwen35MoeLMHeadModel


def huggingface(model_config: Qwen35MoeConfig, quantization: Quantization) -> ExternMapping:
    model = Qwen35MoeLMHeadModel(model_config)
    ...
    for i in range(model_config.num_hidden_layers):
        ...
        mlc_mlp = f"model.layers.{i}.mlp.shared_expert"
        hf_mlp = f"{hf}.layers.{i}.mlp.shared_expert"
        ...
        hf_emlp = f"{hf}.layers.{i}.mlp.experts"
        mlc_name = f"{mlc_mlp}.moe_gate_up_proj.weight"
        if mlc_name in named_parameters:
            mlc_param = named_parameters[mlc_name]
            mapping.add_mapping(
                mlc_name,
                [f"{hf_emlp}.gate_up_proj"],
                functools.partial(lambda x, dtype: x.astype(dtype), dtype=mlc_param.dtype),
            )
        hf_emlp = f"{hf}.layers.{i}.mlp.experts"
        mlc_name = f"{mlc_mlp}.moe_down_proj.weight"
