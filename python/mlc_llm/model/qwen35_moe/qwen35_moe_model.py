...
from mlc_llm.model.qwen35.qwen35_model import Qwen35Config
...
class Qwen35MoeConfig(Qwen35Config):  # pylint: disable=too-many-instance-attributes
...
class Qwen35MoeEmbedding(nn.Embedding):
...
class Qwen35MoeMLP(nn.Module):
    def __init__(self, config: Qwen35MoeConfig, intermediate_size: Optional[int] = None):
...
class Qwen35MoeSparseMoeBlock(nn.Module):  # pylint: disable=too-many-instance-attributes
    """MoE layer for Qwen35MoE model."""
    ...
    def forward(self, x: Tensor):
        ...
        if seq_len == 1:
            # x: [batch_size, num_tokens * experts_per_tok, hidden_size]
...
class Qwen35MoeGatedDeltaNet(nn.Module):
    ...
    def __init__(self, config: Qwen35MoeConfig, linear_layer_idx: int):
        self.config = config
    ...
    def forward(
        ...
        out_recurrent, state_out_layer = op_ext.get_gated_delta_net_op(
            q, k, v, gate, beta, state_in_layer, self.dtype
        )
    ...
    def _causal_conv1d_with_state(self, qkv: Tensor, conv_state: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # Depthwise conv
        result = op_ext.convolution.get_depthwise_conv_with_state_op(
...
class Qwen35MoeDecoderLayer(nn.Module):
    def __init__(self, config: Qwen35MoeConfig, layer_id: int, linear_layer_idx: int = -1):
        self.layer_type = config.layer_types()[layer_id]
        if self.layer_type == "full_attention":
            self.self_attn = Qwen35MoeAttention(config)
        else:
            self.linear_attn = Qwen35MoeGatedDeltaNet(config, linear_layer_idx)
...
class Qwen35MoeModel(nn.Module):
    def __init__(self, config: Qwen35MoeConfig):
        self.embed_tokens = Qwen35MoeEmbedding(config.vocab_size, config.hidden_size)
        layer_types = config.layer_types()
        linear_idx = 0
        layers = []
        for i in range(config.num_hidden_layers):
            if layer_types[i] == "linear_attention":
                layers.append(Qwen35MoeDecoderLayer(config, i, linear_layer_idx=linear_idx))
                linear_idx += 1
            else:
                layers.append(Qwen35MoeDecoderLayer(config, i))
...
class Qwen35MoeLMHeadModel(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: Qwen35MoeConfig):
        self.config = config
